...
class PSI_RiF:
    ...
    def __calcNextStim(self):
        ...
        # M: Determine next stimulus only on the next trial, or more
        if self.stim_selection == 'adaptive':
            ...
            self.stim1_index, self.stim2_index = self.__calcAdaptiveStimMultipleTrials(ps, pf)
        elif self.stim_selection == 'backward':
            self.stim1_index, self.stim2_index = self.__calcBackwardInduction(ps, pf)
        else:
    ...
    def __calcAdaptiveStimMultipleTrials(self, ps, pf):
        # for x in xrange(self.ntrials):

        # print('Current trials is', self.ntrials)

        # cannot take the log of 0
        self.paxs[self.paxs == 0.0] = 1.0e-10
        self.paxf[self.paxf == 0.0] = 1.0e-10

        # M: compute expected entropy for each stimulus X at trial k+1
        hs = np.einsum('ijk,ijk->jk', -self.paxs, np.log(self.paxs))
        hf = np.einsum('ijk,ijk->jk', -self.paxf, np.log(self.paxf))
        h = ps * hs + pf * hf
        # M: Add all expected entropies for each stimulus X to a table entropyLookUp of trial k+1
        self.entropyLookUp.append(h)

        for x in xrange(self.entropyLookUp.__len__()):
            # M: go/increment one trial further
            self.ntrials += 1
            # print('Current trials is', self.ntrials)
            self.paxs[self.paxs == 0.0] = 1.0e-10
            self.paxf[self.paxf == 0.0] = 1.0e-10
            hs = np.einsum('ijk,ijk->jk', -self.paxs, np.log(self.paxs))
            hf = np.einsum('ijk,ijk->jk', -self.paxf, np.log(self.paxf))
            # M: calculate the expected entropy but now for stimulus X at trial k+1
            h2 = ps * hs + pf * hf
            # M: multiply the expected entropy at k+2 with the expected entropy at k+1 (each table element)
            h3 = h2 * self.entropyLookUp[x]

        # M: Determine the stimulus with smallest expected product (k+1 and k+2) entropy
        return np.unravel_index(h3.argmin(), h3.shape)

    def __calcBackwardInduction(self, ps, pf):

        # M: k, the k-trial-ahead-amount optimization.
        k = 2

        # M: i, go i amount of trials backwards
        i = 1

        for x in xrange(self.ntrials):

            # M: Compute for all possible states in the i-to-last trial, or trial t + k - i
            # the optimal stimulus using one-trial ahead optimization.

            self.ntrials = self.ntrials + k - i
            print('ntrials val', self.ntrials)
            ...
            hf = np.einsum('ijk,ijk->jk', -self.paxf, np.log(self.paxf))
            h = ps * hs + pf * hf
            # M: Add all expected entropies for each stimulus X to a table
            self.backwardTable.append(h)

            # i += 1

            for x in xrange(self.ntrials):
                for z in xrange(self.entropyLookUp.__len__()):
                    # M: increment i
                    i += 1

                    # M: Compute for all possible states in the i-to-second-to-last trial, or trial t + k - i -1
                    # the optimal stimulus using one-trial ahead optimization.
                    self.ntrials = self.ntrials + k - i
                    print('ntrials val', self.ntrials)
                    ...
                    # M: Compute expected entropy for t + k - (i-1) by using
                    # the table previously filled to find the optimal stimulus.
                    hs = np.einsum('ijk,ijk->jk', -self.paxs, np.log(self.paxs))
                    hf = np.einsum('ijk,ijk->jk', -self.paxf, np.log(self.paxf))
                    h = ps * hs + pf * hf * self.entropyLookUp[x]
...
#   tab vooruit?
    ...
    def calcParameterDistributions(self):
        # M: SELF NOTE: check here wrt m in einsum
